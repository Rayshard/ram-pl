...
namespace ramvm {
	...
	private:
		...
		int instrPtr, globalPtr;
	public:
		VM(int _maxMemory, const std::vector<Instruction*>& _instrs, const std::unordered_map<std::string, int>& _labels);
		~VM();

		ResultType Run(ResultInfo& _info);
		ResultType ReadFromSrcArg(Argument* _src, DataVariant& _value, ResultInfo& _info);
		ResultType ReadFromSrcArg(Argument* _arg, byte* _buffer, int _length, ResultInfo& _info);
		ResultType WriteToDestArg(Argument* _dest, DataVariant _value, ResultInfo& _info);
